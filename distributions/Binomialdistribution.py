...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        Returns:
            None
        """
        plt.bar(['0', '1'], [len([i for i in self.data if i == 0]), len([i for i in self.data if i == 1])])
        
        plt.title("Bar plot for binomial distribution")
    ...
    def pdf(self, k):
        ...
        """
        n_k = math.factorial(self.n) / (math.factorial(k) * math.factorial(self.n - k))
        pdf_out = n_k * (self.p**k) * ((1 - self.p)**(self.n - k))
    ...
    def __repr__(self):
        ...
        result = "mean {}, standard deviation {}, p {}, n {}".format(self.mean, self.stdev, self.p, self.n)
        return result
